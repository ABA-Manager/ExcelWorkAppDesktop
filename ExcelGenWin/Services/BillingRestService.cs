...
namespace ABABillingAndClaim.Services
{
    public class BillingRestService
    {
        ...
        public BillingRestService(MemoryService memoryService)
        {
        ...
        public async Task<List<ExtendedPeriod>> GetPeriodsAsync()
        {
            ...
            {
                return JsonConvert.DeserializeObject<IEnumerable<ExtendedPeriod>>(response.Content).Where(x => !x.Id.Equals(_memoryService.LoggedOndUser.id)).ToList();
            }
            else
                return null;
        }
